...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        avgScoreLbl.width = W;
        avgScoreLbl.height = W/2;
        avgScoreLbl.centerx = rotateImg.centerx;
        avgScoreLbl.centery = rotateImg.centery+15;
        avgScoreLbl.font=[UIFont systemFontOfSize:PAGESIZE(13)];
        //avgScoreLbl.font = [UIFont fontWithName:@"DIN Alternate" size:13];
        //avgScoreLbl.text = @"56%";
        ...
        descriptionLbl.width = W;
        descriptionLbl.height = W/2;
        descriptionLbl.centerx = rotateImg.centerx;
        descriptionLbl.centery = rotateImg.centery-5;
        descriptionLbl.font = [UIFont systemFontOfSize:PAGESIZE(20)];
...
#pragma mark - **************** 赋值方法
- (void)setPrecent:(int)precent descText:(NSString*)dtext descType:(int)dtype
{
    _precent = precent;
    if (dtype==1) {
        self.avgScoreLbl.text = [NSString stringWithFormat:@"%d％",precent];
    } else {
        self.avgScoreLbl.text = dtext;
    }
}
...
- (void)setPrecent:(int)precent description:(NSString *)description descText:(NSString*)dtext descType:(int)dtype textColor:(UIColor *)tcolor DescTextColor:(UIColor*)dcolor bgColor:(UIColor *)bColor alpha:(CGFloat)alpha clips:(BOOL)clips
{
    [self setAlpha:alpha];
    [self setPrecent:precent descText:dtext descType:dtype];
    [self setClips:clips];
...
- (void)setTextColor:(UIColor *)textColor{
    ...
    //self.avgScoreLbl.textColor = textColor;
}

- (void)setDescTextColor:(UIColor *)textColor{
    self.avgScoreLbl.textColor = textColor;
}
